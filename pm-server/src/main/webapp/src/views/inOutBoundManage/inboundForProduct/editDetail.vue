<template>
  <div class="main">
    <div>{{ $t('route.editInboundOrderProductDetail') }}</div>
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
...
import {
  updateInboundOrderProductDetail,
  getInboundOrderProductDetail
} from "@/api/inboundOrderProductDetail";
...
export default {
  ...
  mounted() {
    ...
    getInboundOrderProductDetail(id).then(res => {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          updateInboundOrderProductDetail(data).then(res => {
            if (res.code === "0") {
              this.$message({
                message: "编辑成功",
                type: "success"
