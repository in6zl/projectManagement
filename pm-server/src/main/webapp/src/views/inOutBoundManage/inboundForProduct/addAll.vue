...
export default {
  ...
  async mounted() {
    ...
    //初始化详情
    if (this.$route.query.productList && this.$route.query.productList.length>0) {
      //相同物料合并
      let productList=[];
      let productMap={};
      this.$route.query.productList.forEach(product=>{
        let productPush=undefined;
        this.productList.forEach(productSingle=>{
          if(productSingle.id==product.productId){
              productPush=productSingle
          }
        })
        if(!productMap[product.productId]){
          productMap[product.productId]={"product":JSON.parse(JSON.stringify(productPush)),"num": product.num,"storageBinNum":1,"storageBins":[]}
        }else{
          productMap[product.productId].num=parseFloat(productMap[product.productId].num) + parseFloat(product.num)
        }
        this.ruleForm.productionRecordIdList.push(product.id)
      })
      for(let key in productMap){
        this.ruleForm.detailList.push(productMap[key])
      }
      console.log('this.ruleForm',this.ruleForm)
      // this.$route.query.productList.forEach(item => {
      //   let productPush=undefined;
    ...
    submitForm(){
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          addInboundOrderProduct(data).then((res) => {
            if (res.code === "0") {
              this.$message({
                message: "创建成功",
                type: "success",
