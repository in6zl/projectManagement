<template>
  <div class="main">
    <div>{{ $t('route.addOutboundOrderProductDetail') }}</div>
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
...
import { addOutboundOrderProductDetail } from "@/api/outboundOrderProductDetail";
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          addOutboundOrderProductDetail(data).then(res => {
            if (res.code === "0") {
              this.$message({
                message: "创建成功",
                type: "success"
