<template>
  <div class="main">
    <div>{{ $t('route.editOutboundOrderProductDetail') }}</div>
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
...
import {
  updateOutboundOrderProductDetail,
  getOutboundOrderProductDetail
} from "@/api/outboundOrderProductDetail";
...
export default {
  ...
  mounted() {
    ...
    getOutboundOrderProductDetail(id).then(res => {
    ...
    submitForm(formName) {
      ...
      const data = {
        ...
        outboundOrderProduct: {
          id: this.$route.query.outProductOrderId
        }
      };
      this.$refs[formName].validate(valid => {
        if (valid) {
          updateOutboundOrderProductDetail(data).then(res => {
            if (res.code === "0") {
              this.$message({
                message: "编辑成功",
                type: "success"
