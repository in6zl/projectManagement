<template>
  <div class="main">
    <div>{{ $t('route.editInboundOrderMaterialDetail') }}</div>
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
...
import {
  updateInboundOrderMaterialDetail,
  getInboundOrderMaterialDetail
} from "@/api/inboundOrderMaterialDetail";
...
export default {
  ...
  mounted() {
    ...
    getInboundOrderMaterialDetail(id).then(res => {
    ...
    submitForm(formName) {
      ...
      const data = {
        ...
        inboundOrderMaterial: {
          id: this.$route.query.sendOrderId
        }
      };
      this.$refs[formName].validate(valid => {
        if (valid) {
          updateInboundOrderMaterialDetail(data).then(res => {
            if (res.code === "0") {
              this.$message({
                message: "编辑成功",
                type: "success"
              ...
              this.$router.push({
                ...
                query: {
                  id: this.$route.query.sendOrderId,
                  sendOrderNo: this.$route.query.sendOrderNo
                }
              });
            } else {
