<template>
  <div class="main">
    <div>{{ $t('route.addInboundOrderMaterialDetail') }}</div>
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
...
import { addInboundOrderMaterialDetail } from "@/api/inboundOrderMaterialDetail";
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          addInboundOrderMaterialDetail(data).then(res => {
            if (res.code === "0") {
              this.$message({
                message: "创建成功",
                type: "success"
