<template>
  <div class="main">
    <div>{{ $t('route.editOutboundOrderMaterialDetail') }}</div>
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
...
import {
  updateOutboundOrderMaterialDetail,
  getOutboundOrderMaterialDetail
} from "@/api/outboundOrderMaterialDetail";
...
export default {
  ...
  mounted() {
    ...
    getOutboundOrderMaterialDetail(id).then(res => {
    ...
    submitForm(formName) {
      ...
      const data = {
        ...
        outboundOrderMaterial: {
          id: this.$route.query.materialRequirePlanId
        }
      };
      this.$refs[formName].validate(valid => {
        if (valid) {
          updateOutboundOrderMaterialDetail(data).then(res => {
            if (res.code === "0") {
              this.$message({
                message: "编辑成功",
                type: "success"
