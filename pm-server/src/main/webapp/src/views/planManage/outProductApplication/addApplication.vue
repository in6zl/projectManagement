  ...
  <div>
    <div class="box">
      <el-scrollbar style="height:100%">
        <div class="main-top">
          <div style="margin:0 0 16px 0" class="subtit">新增出货申请单</div>
          <!--父表单-->
...
<script>
import { addApplication } from "@/api/outProductApplication";
  ...
  mounted() {
    ...
    getDeliver(id).then((res) => {
      if (res.code === "0") {
        ...
        this.ruleForm.deliveryPlanId = res.data.id;
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.ruleForm.detailsList = this.tableData;
          ...
          addApplication(this.ruleForm).then((res) => {
            if (res.code === "0") {
              this.$message({
                message: "创建成功",
                type: "success",
