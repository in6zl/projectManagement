<template>
  <div>
    <con>
      <div class="main-top">
        <div style="margin:0 0 16px 0" class="subtit">新增组织</div>
        <el-form
          :model="ruleForm"
          :rules="rules"
          ref="ruleForm"
        ...
        >
          ...
          <!--        <el-select style="display: block" v-model="ruleForm.type" filterable placeholder="请选择">-->
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          createOrganization(saveOrganization).then((res) => {
            if (res.code === "0") {
              this.$message({
                message: "创建成功",
                type: "success",
