<template>
  <div>
    ...
    <con>
      <div class="main-top">
        <div style="margin:0 0 16px 0" class="subtit">添加生产计划</div>
        <el-form
          ref="planItemForm"
          :rules="planItemRules"
          :model="planItemForm"
    ...
    <submit>
      ...
      <el-button size="small" type="primary" @click="goback('planItemForm')">确定</el-button>
    </submit>
  </div>
</template>
...
export default {
  ...
  methods: {
    initUnit(){
      this.planItemForm.unit={"id":this.planItemForm.product.unitId,"code":this.planItemForm.product.unitCode}
    },
    alertProDialog() {
    ...
    confirm() {
      ...
      let confirmObj = {
        productionPlanItem: this.submitObj,
    ...
    addItem(formName) {
      ...
      this.$refs[formName].validate((validate) => {
        if (validate) {
          ...
          addObj.unit.id = this.planItemForm.unit.id;
          addObj.unit.code = this.planItemForm.unit.code;
    ...
    unitSelect(item) {
      this.planItemForm.unit.id = item.id;
      this.planItemForm.unit.code = item.code;
    },
  },
  mounted() {
    let unitData = {
    ...
    // getSaleNo().then((res) => {
    //   if (res.code == 0) {
    //     this.planItemForm.code = Number(res.data);
    //   }
    // });
    // planItemForm: {
    //     code:"",
