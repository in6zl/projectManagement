  ...
  <div class="main" style="padding:0;">
    ...
    >
      <el-input
        ...
        placeholder="请输入关键字"
        style="width:300px;position:absolute;left:40px;top:40px"
      ></el-input>
      <div style="font-weight:700;margin:30px 0px 20px 20px;font-size:14px">新增内仓</div>

      <el-form-item label="内仓名称" prop="name" size="small" style="padding:10px 0px">
      ...
      <el-form-item label="所属组织" prop="orgList" size="small">
        <el-select v-model="ruleForm.orgList" placeholder="请选择" style="width:100%" multiple collapse-tags>
          <el-option
      ...
      <el-form-item label="所属用户" prop="userList" size="small">
        <el-select v-model="ruleForm.userList" placeholder="请选择" style="width:100%" multiple collapse-tags>
          <el-option
                  v-for="item in users"
                  :key="item.id"
                  :label="item.realName"
...
import { addFaWarehouse, findByLnglat,getByName } from "@/api/faWareHouse";
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          addFaWarehouse(this[formName]).then(res => {
            this.$message({
              message: "创建成功",
              type: "success"
            });
