<template>
  <div>
    ...
    <con>
      <div class="main-top">
        <div style="margin:0 0 16px 0" class="subtit">新增库位</div>
        <el-form
          :model="ruleForm"
          :rules="rules"
          ref="ruleForm"
        ...
        >
          ...
          <el-form-item label="货架" prop="shelvesId" size="small">
...
import { getFaStorageBinList, findFaStorageBin } from "@/api/faStorageBin";
import { addFaStorehouse, getByName } from "@/api/faStorehouse";
import { findBaseInfoList } from "@/api/material";
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          addFaStorehouse(this[formName]).then((res) => {
            this.$message({
              message: "创建成功",
              type: "success",
            });
    ...
    getFaStorageBinList() {
      let faStorageBinObj = { warehouseId: this.ruleForm.warehouseId };
      getFaStorageBinList(faStorageBinObj).then((res) => {
        if (res.code === "0") {
          this.ruleForm.shelvesId = "";
          this.ruleForm.faStorageBinList = res.data;
        }
