<template>
  <div>
    ...
    <submit class="subBtn">
      ...
      <el-button @click="submitForm('ruleForm')" size="small" type="primary">{{$t('button.confirm')}}</el-button>
    </submit>
  </div>
</template>
<script>
...
import { addRegistration, list } from "@/api/registration";
...
export default {
  ...
  data() {
    return {
      sendOrderId: this.$route.query.item,
      ruleForm: {
        carNo: "",
        driverName: "",
        driverPhone: "",
  ...
  mounted() {
    ...
    getValue("登记车辆类型").then(res => {
      ...
      findSendOrder(this.sendOrderId).then(res => {
        let sendOrderNo = res.data.code;
        let deliveries = res.data.invoiceDeliveryInfos;
        deliveries.forEach(delivery => {
          delivery["sendOrderNo"] = sendOrderNo;
        });
        this.tableData = deliveries;
        this.ruleForm.carNo = res.data.carNo;
        this.ruleForm.driverName = res.data.driverName;
        this.ruleForm.driverPhone = res.data.driverPhone;
      });
    });
  },
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.ruleForm.deliveryDTOList.length == 0) {
            this.$message("请选择需要登记的送货清单");
            return;
          }
          ...
          addRegistration(this.ruleForm).then(res => {
            if (res.code === "0") {
              this.$message({
                message: "登记成功!",
                type: "success"
