<template>
  <div>
    ...
    <con>
      <div class="main-top">
        <div style="margin:0 0 16px 0" class="subtit">编辑标签</div>
        <el-form
          :model="ruleForm"
          :rules="rules"
          ref="ruleForm"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        priority: [{ validator: numValidator, trigger: "blur" }],
      },
      formName: "",
    };
  },
  mounted() {
    ...
    findFaWarehouseTag(id).then((res) => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          updateFaWarehouseTag(this[formName]).then((res) => {
            this.$message({
              message: "更新成功",
              type: "success",
            });
