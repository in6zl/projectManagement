      ...
      <div class="main-top">
        ...
        >
          ...
          <el-form-item label="金额" prop="amount">
            <el-input :placeholder="$t('main.placeInsert')" size="small" v-model="saleForm.amount"></el-input>
          </el-form-item>
          ...
          <el-form-item label="期望到货时间" prop="expectDate">
            <el-date-picker
              v-model="saleForm.expectDate"
              type="date"
              placeholder="选择日期时间"
              size="small"
        ...
        <div class="roleManageTable" v-if="active==1">
          ...
          >
            ...
            <el-table-column prop="unit" label="单位" width="135" :show-overflow-tooltip="true"></el-table-column>
...
import { updateSellOrder, find, getByName } from "@/api/sellOrder";
...
export default {
    ...
    return {
      ...
      saleRules: {
        orderNo: [
          ...
          { type: "number", message: "订单编号必须是数字", trigger: "change" },
          {
  ...
  methods: {
    ...
    handleScroll() {
      this.$refs.btn.style.bottom =
        -document.querySelector(".main").scrollTop + "px";
    },
    goHome() {
    ...
    success() {
      ...
      data.expectDate = parseTime(this.saleForm.expectDate);
  ...
  mounted() {
    ...
    let id = this.$route.query.id;
    find(id).then((res) => {
      this.saleForm.seller = res.data.seller;
      this.saleForm.sellerPhone = res.data.sellerPhone;
      this.saleForm.expectDate = res.data.expectDate;
      this.saleForm.amount = res.data.amount;
      this.saleForm.orderTime = res.data.orderTime;
      this.saleForm.shipment = res.data.address;
      this.saleForm.orderNo = Number(res.data.orderNo);
      this.tableData = res.data.saleDetailsList;
    });
