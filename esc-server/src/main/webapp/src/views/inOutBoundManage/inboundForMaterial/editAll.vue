...
export default {
  ...
  async mounted() {
    ...
    await getInboundOrderMaterial(this.$route.query.id).then(res => {
      if(res.data.invoice && res.data.invoice.id){
        res.data['invoiceId']=res.data.invoice.id;
      }
      let inboundOrderMaterial=res.data;
      let detailMap={};
      inboundOrderMaterial.detailList.forEach(detail=>{
        this.materialMap[detail.material.id]=detail.material;
        this.storageBinMap[detail.bin.id]=detail.bin;
        detail.bin.inboundNum=detail.num;
        detail.bin.batch=detail.batch;
        if(!detailMap[detail.material.id]){
          detailMap[detail.material.id]={
            "material":detail.material,
            "num":parseFloat(detail.num),
            "storageBinNum":1,
            "storageBins":[]
          }
          detailMap[detail.material.id].storageBins.push(detail.bin)
        }else{
          detailMap[detail.material.id].num=parseFloat(detailMap[detail.material.id].num)+detail.num;
          detailMap[detail.material.id].storageBins.push(detail.bin)
        }
      })
      let detailList=[];
    ...
    async selectWarhouse(){
      ...
      await getFaStorehouseList({"warehouseId":this.ruleForm.warehouse.id}).then(res=>{
         res.data.forEach(storageBin=>{
           ...
           //初始化库位时，由于数据库状态为2，但实际为自己入库单锁定，所以先假定设置其为0
